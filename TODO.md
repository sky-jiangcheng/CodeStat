...
> 测试补齐、`useApiData` 接入（NoteSection/CommandPalette，共享 `projects:all` 缓存）、`wails.json` 清理均已完成，见 CHANGELOG Unreleased。
...
> `examples/plugins` 两个示例插件回归校验已完成（宿主 `internal/core/plugin` SPI 未变，`go build ./...` 通过），见 CHANGELOG Unreleased。
